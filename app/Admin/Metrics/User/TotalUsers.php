...
//        $this->height(150);
    ...
    public function handle(Request $request)
    {
        ...
        $total = User::query()->where('pid',$referrer)->count();
        $total_auth = User::query()->where('pid',$referrer)->where('user_auth_level',User::user_auth_level_top)->count();
        $today = User::query()->where('pid',$referrer)->whereDate('created_at',$today_date)->count();
        $today_auth = User::query()->where('pid',$referrer)->whereDate('created_at',$today_date)->where('user_auth_level',User::user_auth_level_top)->count();
...
<div class="d-flex p-1 flex-column justify-content-between" style="padding-top: 0;width: 100%;height: 100%;min-height: {$minHeight}">
    ...
    <a href="/admin/users" class="btn btn-primary shadow waves-effect waves-light">View Details <i class="feather icon-chevrons-right"></i></a>
</div>
HTML
        );
    }
