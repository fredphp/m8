...
class ResetUserAuth extends BatchAction
{
...
//            if($auth->status != UserAuth::STATUS_AUTH){
//                return $this->response()->error('Processed fail.')->refresh();
//            }

            $auth->status = 0;
            $auth->primary_status = 0;
            $auth->save();
            $user = User::query()->find($auth->user_id);
            $user->user_auth_level = User::user_auth_level_wait;
            $user->save();
        }
