...
//            $q->where('is_system',0);
        ...
        if (Admin::user()->inRoles(['员工', '经理级'])) {
            ...
            $builder1->whereIn('user_id',$lowerIds);
        };
...
//                        $actions->append(new cancel());
            ...
            if (!Admin::user()->inRoles(['超级管理员', '经理级'])) {
...
//            $grid->column('contract_coin_id');
            ...
            $grid->column('type','类型')->using(ContractEntrust::$typeMap);
            $grid->column('lever_rate','杠杆');
            $grid->column('entrust_price','委托价');
//            $grid->column('trigger_price');
            $grid->column('amount','委托张数');
            $grid->column('traded_amount','成交张数');
//            $grid->column('margin');
            $grid->column('avg_price','均价');
            $grid->column('fee','手续费');
            $grid->column('profit','盈亏');
            $grid->column('settle_profit','结算盈亏');
