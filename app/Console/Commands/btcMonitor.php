<?php

namespace App\Console\Commands;

use App\Models\Mongodb\btcTransactions;
use App\Models\Recharge;
use App\Models\User;
use App\Models\UserAddress;
use App\Models\UserWallet;
use App\Services\CoinService\Libs\BitcoinClient;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Cache;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class btcMonitor extends Command
{
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'btcMonitor';

    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'Command description';

    /**
     * Create a new command instance.
     *
     * @return void
     */
    public function __construct()
    {
        parent::__construct();
    }

    /**
     * Execute the console command.
     *
     * @return mixed
     */
    public function handle()
    {
        $client = new BitcoinClient(env('OMNICORE_USERNAME'), env('OMNICORE_UPASSWORD'), env('OMNICORE_HOST'), env('OMNICORE_PORT'));
        while (true) {
            try {
                $transactions = $client->listtransactions("*", 30);
                if (!$transactions){
                    $transactions = [];
                }
            } catch (\RuntimeException $e) {
                info($e);
                $transactions = [];
            }

            echo count($transactions) . '--' . datetime() . "\r\n";
            if (count($transactions) > 0) {
                // 监听地址列表
//                $address_list = \App\Models\UserWallet::query()->where('coin_id', 2)->where(function ($q) {
//                    $q->whereNotNull('wallet_address')->where('wallet_address', '<>', '');
//                })->pluck('wallet_address')->toArray();
                $address_list = UserAddress::where('status', 1)->where('user_id', '!=', 0)->pluck('omni_address')->toArray();

                foreach ($transactions as $transaction) {
                    if ($transaction['category'] == 'receive' && $transaction['confirmations'] >= 6 && isset($transaction['address'])) {
                        if (in_array($transaction['address'], $address_list)) {
                            $txid = $transaction['txid']; // 交易hash
                            // 确认交易是否已经存在;
                            $is_exist = Recharge::query()->where('txid', $txid)->first();
                            if ($is_exist) {
                                // 已存在
//                                $data = $is_exist->toArray();
//                                if ($data['status'] == 0) \App\Jobs\Deposit::dispatch($data)->onQueue('deposit');
                                continue;
                            } else {
                                $amount = custom_number_format($transaction['amount'], 8);
//                                $data = $transaction;
//                                $txid = $transaction['txid'];
//                                $data['to'] = $transaction['address'];
//                                $data['amount'] = $amount;
//                                $data['symbol'] = 'BTC';
//                                $data['type'] = 'deposit';
                                if ($amount >= 0.000001) {
                                    // 交易金额太小 则被认为是交易手续费 不作为充值记录
//                                    $data['status'] = 0;
//                                    \App\Jobs\Deposit::dispatch($data)->onQueue('deposit');
                                    //上账处理
                                    DB::beginTransaction();
                                    try {
                                        $userAddress = UserAddress::where('omni_address', $transaction['address'])->first();
                                        // 更新用户余额
                                        $user = User::query()->findOrFail($userAddress->user_id);
                                        $user->update_wallet_and_log(2, 'usable_balance', $amount, UserWallet::asset_account, 'recharge');
                                        $cachePrice = Cache::store('redis')->get('swap:trade_detail_BTC') ?? ['price' => 0];
                                        $cur_price = $cachePrice['price'];
                                        // 记录日志
                                        Recharge::query()->create([
                                            'user_id' => $user['user_id'],
                                            'username' => $user['username'],
                                            'coin_id' => 2,
                                            'coin_name' => 'BTC',
                                            'datetime' => time(),
                                            'address' => $transaction['address'],
                                            'txid' => $transaction['txid'],
                                            'amount' => $amount,
                                            'status' => Recharge::status_pass,
                                            'note' => '充值到账',
                                            'cur_price' => $cur_price
                                        ]);
                                        DB::commit();
                                    } catch (\Exception $exception) {
                                        Log::info('比特币上账失败 交易哈希:'.$transaction['txid']);
                                        DB::rollback();
                                    }
                                }
//                                else {
//                                    $data['status'] = 1;
//                                }
//                                btcTransactions::query()->create($data);
                            }
                        }
                    }
                }
            }
            sleep(5);
        }

    }
}
