...
class TrcListen extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'trc-listen {user_id?}';
     ...
     * @var string
     */
    protected $description = '监听波场上的USDT到账';
    ...
    {
        $contracts = ['TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t', 'TEkxiTehnzSmSe2XqrBj4w32RUN966rdz8']; //USDT USDC
        ...
        foreach ($addressList as $address) {
            ...
            foreach ($contracts as $contract) {
                $url = 'https://trx-blockbook.nownodes.io/api/v2/address/' . $address->omni_address . '?details=txs&contract=' . $contract;
            ...
            foreach ($data as $item) {
                ...
                $rechargeAddress = $address->omni_address;
                if (strtolower($item['tokenTransfers'][0]['to']) != strtolower($rechargeAddress)){//只要入账的
                    continue;
                }
                $amount = bcdiv($item['tokenTransfers'][0]['value'], bcpow(10, 6), 6);
                //上账处理
                DB::beginTransaction();
                try {
                    // 更新用户余额
