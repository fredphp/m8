                ...
                try {
                    ...
                    if($user){
                        if($k['two_coin_id'] > 0){
                            ...
                            $user->update_wallet_and_log($k['coin_id'], 'freeze_balance', -$k['one_num'],
                            UserWallet::asset_account, 'sell_pledge_product');
                            // 发行币
                            $user->update_wallet_and_log($k['two_coin_id'], 'usable_balance', $k['two_num'],
                            UserWallet::asset_account, 'sell_pledge_product');
                            $user->update_wallet_and_log($k['two_coin_id'], 'freeze_balance', -$k['two_num'],
                            UserWallet::asset_account, 'sell_pledge_product');
                        }else{
                            $user->update_wallet_and_log($k['coin_id'], 'usable_balance', $k['total'],
                            UserWallet::asset_account, 'sell_pledge_product');
