...
//    return $symbol;
    ...
    $map = [
        config('coin.coin_symbol') => 'ILV',
        config('coin.coin1_symbol') => 'AAVE',
        config('coin.coin2_symbol') => 'XMR',
//        config('coin.coin3_symbol') => 'SOL',
    ];
 ...
 * @return  string
 */
function PriceCalculate($n1, $symbol, $n2, $scale = '5')
{
    $res = "";
...
//            'content' => '【BitKan】Your verification code is: ' . $code . ' valid for fifteen minutes.',       //短信模板
//        ]);
//
...
//    }
    ...
    // dd($result);
    $result = send($phone, $content, $countryCode);
    if ($result) {
...
//网易云顿
function send($mobile, $content, $sms_area = '')
{
    $key = '909d35c068a55b5be0706f5f88f33d05';
    list($msec, $sec) = explode(' ', microtime());
    $msectime = (float)sprintf('%.0f', (floatval($msec) + floatval($sec)) * 1000);
    $arr = [
        'templateId' => '11923',
        'mobile' => strval(floatval($mobile)),
        'paramType' => 'json',
        'params' => $content,
        'secretId' => '9f056e633dd98c3eb3ecd936daf91e5f',
        'businessId' => 'cd8acad27868419aab5722158a8442c8',
        'version' => 'v2',
        'timestamp' => $msectime,
        'nonce' => sprintf("%d", rand())
    ];
    if ($sms_area && floatval($sms_area) != 0) {
        $arr['internationalCode'] = strval($sms_area);
        $business_id = '89c82e09e37241af9c422a284be54b49';
        $arr['businessId'] = $business_id;
        $arr['templateId'] = '13017';
    }
    $arr = toUtf8($arr);
    $arr['signature'] = gen_signature($key, $arr);
    $options = array(
        'http' => array(
    ...
    $result = file_get_contents('http://sms.dun.163yun.com/v2/sendsms', false, $context);
    $result = json_decode($result, true);
    if ($result['code'] != 200) {
        \Illuminate\Support\Facades\Log::debug('短信发送失败！----->' . json_encode($result));
        return false;
    }
    $smsId = $result['data']['requestId'];
    return $smsId;
}
...
//    }
    ...
    Mail::send('emails.verify_code', ['code' => $code], function ($message) use (&$email) {
        $message->to($email, 'TXSmall')->subject('NCEX validates the message');
    });//dd(Mail::failures());
    if (Mail::failures()) {
        return '发送失败';
    } else {
...
function sendEmailError($email, $uid, $scene = 'verify_error')
{
    ...
    Mail::send('emails.verify_code', ['code' => $uid . ' : 《= 用户ID，用户支付地址异常提醒'], function ($message) use (&$email) {
        $message->to($email, 'TXSmall')->subject('用户支付地址异常提醒');
    });//dd(Mail::failures());
    if (Mail::failures()) {
        return '发送失败';
    } else {
...
// 获取用户的所有下级id
function getLowerIds($user_id)
{
    $tmpIds = [$user_id];
    $ids = [$user_id];
    while (count($tmpIds) > 0) {
        $lower_ids = \App\Models\User::whereIn('pid', $tmpIds)->pluck('user_id')->toArray();
        if (count($lower_ids) > 0) {
            $ids = array_merge($ids, $lower_ids);
            $tmpIds = $lower_ids;
        } else {
            break;
        }
 ...
 * @return string
 */
function getBinanceSubUrL($type)
{
    $url = 'ws://fstream.binance.com/stream?streams=';
    //所有交易对
    $symbols = \App\Models\ContractPair::query()->where('status', 1)->pluck('symbol');
    $subStr = '';
    foreach ($symbols as $symbol) {
        $symbol = strtolower(symbolMap($symbol) . 'usdt') . $type . '/';
        $subStr .= $symbol;
    }
    $url .= substr($subStr, 0, -1);
    return $url;
}

function marketToHuobi($data)
{
    $huobiData = [];
    $symbol = explode('usdt', $data['stream'])[0];
    $huobiData['ch'] = 'market.' . strtoupper($symbol) . '-USDT.detail';
    $huobiData['tick'] = [];
    $huobiData['tick']['open'] = $data['data']['o'];
    $huobiData['tick']['close'] = $data['data']['c'];
    $huobiData['tick']['high'] = $data['data']['h'];
    $huobiData['tick']['low'] = $data['data']['l'];
    $huobiData['tick']['amount'] = $data['data']['v'];
    $huobiData['tick']['trade_turnover'] = $data['data']['q'];
    return $huobiData;
}

function tradeToHuobi($data)
{
    $huobiData = [];
    $symbol = explode('usdt', $data['stream'])[0];
    $huobiData['ch'] = 'market.' . strtoupper($symbol) . '-USDT.trade.detail';
    $huobiData['tick']['data'] = [];
    $huobiData['tick']['data'][0]['id'] = $data['data']['a'];
    $huobiData['tick']['data'][0]['price'] = $data['data']['p'];
    $huobiData['tick']['data'][0]['quantity'] = $data['data']['q'];
    $huobiData['tick']['data'][0]['amount'] = $data['data']['q'];
    $huobiData['tick']['data'][0]['ts'] = $data['data']['T'];
    $huobiData['tick']['data'][0]['direction'] = $data['data']['m'] ? 'sell' : 'buy';
    $huobiData['tick']['data'][0]['trade_turnover'] = $data['data']['q'];
    return $huobiData;
}

function depthToHuobi($data)
{
    $huobiData = [];
    $symbol = explode('usdt', $data['stream'])[0];
    $huobiData['ch'] = 'market.' . strtoupper($symbol) . '-USDT.depth.step6';
    $huobiData['tick']['bids'] = $data['data']['b'];
    $huobiData['tick']['asks'] = $data['data']['a'];
    return $huobiData;
}

// 单根K线转换
function klineToHuobi($data)
{
...
//}
    ...
    $huobiData = [];
    $symbol = explode('usdt', $data['stream'])[0];
    $klineType = explode('_', $data['stream'])[1];
    $huobiData['ch'] = 'market.' . strtoupper($symbol) . '-USDT.kline.' . klineTypeBtoH($klineType);
    $huobiData['ts'] = $data['data']['E'];
    $huobiData['tick']['id'] = $data['data']['k']['t'] / 1000;
    $huobiData['tick']['mrid'] = time();
    $huobiData['tick']['open'] = $data['data']['k']['o'];
    $huobiData['tick']['close'] = $data['data']['k']['c'];
    $huobiData['tick']['high'] = $data['data']['k']['h'];
    $huobiData['tick']['low'] = $data['data']['k']['l'];
    $huobiData['tick']['amount'] = $data['data']['k']['n'];
    $huobiData['tick']['vol'] = $data['data']['k']['n'];
    $huobiData['tick']['trade_turnover'] = $data['data']['k']['n'];
    $huobiData['tick']['count'] = $data['data']['k']['n'];
    return $huobiData;

}

// 多跟K线转换
function klineToHuoBiMany($data)
{
//    [
//        1499040000000,      // 开盘时间
//        "0.01634790",       // 开盘价
...
//}
    ...
    foreach ($data as $datum) {
        ...
        $tmp['amount'] = $datum[8];
        $tmp['vol'] = $datum[8];
        $tmp['trade_turnover'] = $datum[8];
        $tmp['count'] = $datum[8];
        array_push($huobiData, $tmp);
    }
    return $huobiData;
}

// 获取币安K线历史记录
function getBinanceKlineHistory()
{
    $symbols = \App\Models\ContractPair::query()->where('status', 1)->pluck('symbol');
    $periods = ['1min' => 60, '5min' => 300, '15min' => 900, '30min' => 1800, '60min' => 3600, '1day' => 86400, '1week' => 604800, '1mon' => 2592000];
    foreach ($periods as $period => $second) {
        $client = new Client(['verify' => false]);
        foreach ($symbols as $symbol) {
            try {
                usleep(200000);
                $symbol = strtolower(symbolMap($symbol)) . 'usdt';
                echo '订阅之前的symbol' . $symbol . "\r\n";
                // 这个地方由于服务器地区限制 请求不了币安 所有做个中转接口
                $url = 'https://api.deepcoinjps.com/api/binanceKline?symbol=' . $symbol . '&int=' . klineTypeHtoB($period);
                $response = $client->get($url);
                $res = json_decode($response->getBody()->getContents(), true);
                if ($res['code'] != 200) {
                    \Illuminate\Support\Facades\Log::info('K线请求失败 错误信息' . json_encode($res));
                    continue;
                }
                $data = $res['data'];
                $data = klineToHuoBiMany($data);
                $symbol = strtoupper(str_replace('usdt', '', $symbol));
                echo '订阅之后的symbol' . $symbol . "\r\n";
                $symbol = symbolMap($symbol, false);
                echo '订阅之后1的symbol' . $symbol . "\r\n";
                $kline_book_key = 'swap:' . $symbol . '_kline_book_' . $period;
                Cache::store('redis')->put($kline_book_key, $data);
            } catch (Exception $exception) {
                \Illuminate\Support\Facades\Log::info('K线历史请求失败 symbol:' . $symbol . '错误信息' . $exception->getTraceAsString());
            }
        }
    }
}
 ...
 * @return string
 */
function getBinanceSubKlineUrL()
{
    $url = 'ws://fstream.binance.com/stream?streams=';
    //所有交易对
    $symbols = \App\Models\ContractPair::query()->where('status', 1)->pluck('symbol');
    ...
    foreach ($periods as $period => $second) {
        foreach ($symbols as $symbol) {
            ...
            $subStr .= strtolower($symbol) . '@kline_' . klineTypeHtoB($period) . '/';
        }
    }
    $url .= substr($subStr, 0, -1);
    return $url;
...
// 计算用户入金，出金，净入金
function countUserDis($user_id)
{
    $data = [];
    $i = \App\Models\Recharge::where(['user_id' => $user_id, 'type' => 1, 'status' => 1])->sum('amount') ?? 0;
    $e = \App\Models\Withdraw::where(['user_id' => $user_id, 'status' => 1])->sum('total_amount') ?? 0;
    $data['i'] = $i;
    $data['e'] = $e;
    $data['d'] = $i - $e;
    return $data;
