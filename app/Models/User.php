...
//            }
    ...
    public function update_wallet_and_log(
        ...
        if(blank($wallet)) throw new ApiException('钱包类型错误');
        //余额类型
        $balance =number_format( $wallet->$rich_type, 8, '.', '');
        $absAmount = number_format(abs($amount), 8, '.', '');
        if ( $amount < 0 && bccomp($balance,$absAmount,8) < 0 ) {

            throw new ApiException('资产不足');
        }


        if($account_type == UserWallet::sustainable_account){ //合约账户
