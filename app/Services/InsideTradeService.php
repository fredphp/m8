...
class InsideTradeService
{
    ...
    public function cancelEntrust($user,$entrust)
    {
        ...
        try{
            ...
            if($entrust['entrust_type'] == 1){
                ...
                $user->update_wallet_and_log($entrust['quote_coin_id'],'usable_balance',$return_money,UserWallet::asset_account,'cancel_entrust',$entrust['symbol']);
                $user->update_wallet_and_log($entrust['quote_coin_id'],'freeze_balance',-$return_money,UserWallet::asset_account,'cancel_entrust',$entrust['symbol']);
            }else{
                //更新用户资产
                $return_money = $entrust['surplus_amount'];
                $user->update_wallet_and_log($entrust['base_coin_id'],'usable_balance',$return_money,UserWallet::asset_account,'cancel_entrust',$entrust['symbol']);
                $user->update_wallet_and_log($entrust['base_coin_id'],'freeze_balance',-$return_money,UserWallet::asset_account,'cancel_entrust',$entrust['symbol']);
            }
    ...
    public function storeBuyEntrust($user,$params)
    {
        ...
        if($params['type'] == 1){ //限价
            ...
            $trigger_price = null;
            $amount = null;
            $money = PriceCalculate($params['total'],'*',1,6);
            $hang_status = 1;
        }else{
            $entrust_price = $params['entrust_price'];
            if($entrust_price <= 0) throw new ApiException('请输入价格');
        ...
        if($balance < $money) throw new ApiException('余额不足');
        if ($money < $pair['min_total']) throw new ApiException('交易额低于最低交易标准');
        ...
        try{
            ...
            //扣除用户可用资产 冻结
            $user->update_wallet_and_log($wallet['coin_id'],'usable_balance',-$money,UserWallet::asset_account,'store_buy_entrust',$pair['pair_name']);
            $user->update_wallet_and_log($wallet['coin_id'],'freeze_balance',$money,UserWallet::asset_account,'store_buy_entrust',$pair['pair_name']);
...
//            $entrust_price = $cacheData['price'];
            $money = PriceCalculate($amount,'*',$cacheData['price'],6);
            $trigger_price = null;
            $hang_status = 1;
        }else{
            $entrust_price = $params['entrust_price'];
        ...
        try{
            ...
            //扣除用户可用资产 冻结
            $user->update_wallet_and_log($wallet['coin_id'],'usable_balance',-$amount,UserWallet::asset_account,'store_sell_entrust',$pair['pair_name']);
            $user->update_wallet_and_log($wallet['coin_id'],'freeze_balance',$amount,UserWallet::asset_account,'store_sell_entrust',$pair['pair_name']);
    ...
    public function handleBuyOrder($tradeBuy)
    {
        ...
        if($tradeBuy['type'] == 1 || $tradeBuy['type'] == 3){
            ...
            $surplus_amount = PriceCalculate($tradeBuy['amount'],'-',$tradeBuy['traded_amount'],8);
            $surplus_money = PriceCalculate($tradeBuy['money'],'-',$tradeBuy['traded_money'],6);
        }else{
            ...
            $entrust_price = $cacheData['price'];
            $surplus_money = PriceCalculate($tradeBuy['money'],'-',$tradeBuy['traded_money'],6);
            $surplus_amount = PriceCalculate($surplus_money,'/',$entrust_price,8);
        }
    ...
    public function handleSellOrder($tradeSell)
    {
        ...
        if($tradeSell['type'] == 1 || $tradeSell['type'] == 3){
            ...
            $surplus_amount = PriceCalculate($tradeSell['amount'],'-',$tradeSell['traded_amount'],8);
            $surplus_money = PriceCalculate($tradeSell['money'],'-',$tradeSell['traded_money'],6)  ;
        }else{
            $cacheKey = 'market:' . strtolower(str_before($tradeSell['symbol'],'/') . str_after($tradeSell['symbol'],'/')) . '_newPrice';
            $cacheData = Cache::store('redis')->get($cacheKey);
            if(blank($cacheData)) return ;
            $entrust_price = $cacheData['price'];
            $surplus_amount = PriceCalculate($tradeSell['amount'],'-',$tradeSell['traded_amount'],8);
            $surplus_money = PriceCalculate($surplus_amount,'*',$entrust_price,6) ;
        }
        if($surplus_amount <= 0) return;
...
//            $sell_fee = PriceCalculate($exchange_money ,'*', $taker_fee_rate,8);
            $buy->update([
                'traded_amount' => PriceCalculate($buy['traded_amount'],'+',$sell['amount'],8),
                'traded_money' => PriceCalculate($buy['traded_money'],'+',$sell['money'],8),
                'status' => empty($deal_amount) ? InsideTradeBuy::status_completed : InsideTradeBuy::status_trading,
//                'entrust_price' => $unit_price
            ]);
            //买单全部成交
            if ($buy['type'] == 2){ //市价
                $buy->update([
...
//                'entrust_price' => $unit_price
            ...
            //卖单全部成交
            $sell->update([
                'traded_amount' => PriceCalculate($sell['traded_amount'],'+',$buy['amount'],8) ,
                'traded_money' => PriceCalculate($sell['traded_money'],'+',$buy['money'],6) ,
                'status' => empty($deal_amount) ? InsideTradeBuy::status_completed : InsideTradeBuy::status_trading,
//                'entrust_price' => $unit_price
            ]);
            if ($sell['type'] == 2){ //市价
        ...
        if($side == 'buy' && $buy_user){
            ...
            $buy_user->update_wallet_and_log($buy['quote_coin_id'], 'usable_balance',PriceCalculate($freeze_balance,'-',$exchange_money,6), UserWallet::asset_account, 'entrust_exchange');

           // $buy_user->update_wallet_and_log($buy['quote_coin_id'],'freeze_balance',-$exchange_money,UserWallet::asset_account,'entrust_exchange');
            $buy_user->update_wallet_and_log($buy['base_coin_id'],'usable_balance',PriceCalculate($exchange_amount,'-',$buy_fee,8) ,UserWallet::asset_account,'entrust_exchange');
        }elseif($side == 'sell' && $sell_user){
            $sell_user->update_wallet_and_log($sell['quote_coin_id'],'usable_balance',PriceCalculate($exchange_money,'-',$sell_fee,6),UserWallet::asset_account,'entrust_exchange');
