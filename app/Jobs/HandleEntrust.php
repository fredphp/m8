...
class HandleEntrust implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if($entrust['entrust_type'] == 1){ //买单处理
    ...
    public function handleBuyTrade($entrust,$sellList)
    {
        ...
        foreach ($sellList as $tradeSell){
            ...
            if($entrust['type'] == 1 || $entrust['type'] == 3){
                ...
                $entrust_surplus_amount = PriceCalculate($entrust['amount'],'-',$entrust_traded_amount,8); //剩余交易量 计量单位
                ...
                $exchange_money = PriceCalculate($exchange_amount,"*",$unit_price,6);
                $entrust_amount = PriceCalculate($entrust_amount,'-',$exchange_amount,8);
                $entrust_money = PriceCalculate($entrust_money,'-',$exchange_money,6) ;
                $entrust_traded_amount = PriceCalculate($entrust_traded_amount,'+',$exchange_amount,8);
                $entrust_traded_money = PriceCalculate($entrust_traded_money,'+',$exchange_money) ;
            }else{
                //买单市价委托 只可与卖单限价委托交易
                $entrust_surplus_money = PriceCalculate($entrust['money'],'-',$entrust_traded_money,6); //剩余交易额 计量单位
                $buy_amount =PriceCalculate($entrust_surplus_money,'/',$tradeSell['entrust_price'],8); //剩余交易量
                ...
                $entrust_money = PriceCalculate($entrust_money,'-',$exchange_money,6) ;
                $entrust_traded_amount = PriceCalculate($entrust_traded_amount,'+',$exchange_amount,8) ;
                $entrust_traded_money = PriceCalculate($entrust_traded_money,'+',$exchange_money,6);
            }

            //更新卖单
            $sell_traded_amount = PriceCalculate($tradeSell['traded_amount'],'+',$exchange_amount,8);
            $sell_traded_money = PriceCalculate($tradeSell['traded_money'],'+',$exchange_money,6) ;
...
//                        $unit_price = max($entrust['entrust_price'],$tradeBuy['entrust_price']);
                ...
                $buy_surplus_amount = PriceCalculate($tradeBuy['money'],'/',$unit_price,8); //计算买单可交易量
            ...
            $buy_traded_amount = PriceCalculate($tradeBuy['traded_amount'],'+',$exchange_amount,8);
            $buy_traded_money = PriceCalculate( $tradeBuy['traded_money'],'+',$exchange_money,6);
