        ...
        try{
            ...
            foreach ($positions as $position){
                ...
                if($position['side'] == 1){
                    ...
                    // 先平仓
                    $entrust = $this->dealManyPosition($position,$this->system); // 处理多仓全平
                }else{
                    $entrust = $this->dealEmptyPosition($position,$this->system); // 处理空仓全平
                }
...
//            $open_position_price = ContractPosition::query()->where(['user_id'=>$entrust['user_id'],'contract_id'=>$entrust['contract_id'],'side'=>$position_side])->value('avg_price');
            $open_position_price = ContractPosition::query()->where('id',$entrust['position_id'])->value('avg_price');
    ...
    private function dealManyPosition($position,$system = 0)
    {
        ...
        // 记录仓位保证金(平仓时直接抵消掉)
        $unit_amount = $position['unit_amount']; // 一手几个币
        // （持仓保证金 / 持仓手数）* 可平仓数
//        $margin = ($position['position_margin'] / $position['hold_position']) * $position['avail_position'];
        $a = PriceCalculate($position['position_margin'],'/',$position['hold_position'],5);
        $margin = PriceCalculate($a,'*',$position['avail_position'],5);
        ...
        $fee = PriceCalculate($entrust_price *  $unit_amount *  $position['avail_position'] / $position['lever_rate'] ,'*', (new ContractService())->getNewFeeScale($position['lever_rate']),5);
        //用户创建平仓订单 多仓 卖出平多
        $order_data = [
            'user_id' => $position['user_id'],
...
//        $unit_fee = PriceCalculate($unit_amount * $unit_price / $position['lever_rate'] ,'*', $pair['maker_fee_rate'],5); // 单张合约手续费
        $buy_traded_amount = $buy['traded_amount'] + $exchange_amount;
        $sell_traded_amount = $sell['traded_amount'] + $exchange_amount;

//        $buy_margin = PriceCalculate(($exchange_amount * $unit_amount * $unit_price) ,'/', $buy['lever_rate'],5);
//        $sell_margin = PriceCalculate(($exchange_amount * $unit_amount * $unit_price) ,'/', $sell['lever_rate'],5);
...
//        $sell_fee = PriceCalculate($exchange_amount ,'*', $unit_fee,5);
        ...
        if(!blank($sell_user)){
            ...
            $many_position->update([
                ...
                'position_margin' => PriceCalculate($many_position['position_margin'],'-', $sell_margin,5),
                'close_order_id' => $sell['id'].',',
                'status' => 2
            ]);
...
//        $unit_fee = PriceCalculate($unit_amount * $entrust_price,'*', $pair['maker_fee_rate'],5); // 单张合约手续费
...
//        $sell_fee = PriceCalculate($exchange_amount ,'*', $unit_fee,5);
        ...
        if(!blank($buy_user)){
            ...
            $empty_position->update([
                ...
                'position_margin' => PriceCalculate($empty_position['position_margin'],'-', $buy_margin,5),
                'close_order_id' => $sell['id'].',',
                'status' => 2
            ]);
