...
class HandleContractEntrust implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $flag = false; // 判断是否立即撮合
        if ($entrust['type'] == 1 || $entrust['type'] == 3) { // 限价单
            // 获取最新一条成交记录 即实时最新价格
        ...
        if ($flag) {
            ...
            // 价格符合市场条件的 没有对手单时 系统成交  order_type[1-开仓 2-平仓]  side[1-开多 2-开空]
            if ($entrust['order_type'] == 1 && $entrust['side'] == 1) {
                // 买入开多
                $service->handleOpenBuyOrder($entrust);  //已兼容
            } elseif ($entrust['order_type'] == 1 && $entrust['side'] == 2) {
                // 卖出开空
                $service->handleOpenSellOrder($entrust); // 已兼容
            } elseif ($entrust['order_type'] == 2 && $entrust['side'] == 1) {  //平仓委托 entrust 会有position_id参数
                // 买入平空
