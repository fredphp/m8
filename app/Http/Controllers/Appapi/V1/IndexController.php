...
class IndexController extends ApiController
{
    ...
    #首页数据
    public function indexList_copy(){
        ...
        $bazaar = Article::query()->where(array("category_id"=>$category_id))->orderByDesc("created_at")->limit(4)->get();
    ...
    public function indexList(){
        ...
        $category_id = ArticleCategory::$typeMap["notice"];
        $bazaar = Article::query()->where(array("category_id"=>$category_id))->orderByDesc("created_at")->limit(1)->get();
        $arr["noticeList"] = $bazaar->makeHidden("translations")->toArray();

        // var_dump(json_encode($bazaar[0]->toArray()));
        $arr["marketList"] = $market;
        $arr["bannerList"] = $banner;
        $arr["brokenLine"] = $brokenLine;
        return $this->successWithData($arr);
    ...
    public function getCollect(Request $request){
        ...
        if( !$result ){
            $result = ['BTC/USDT','ETH/USDT','SOL/USDT','LTC/USDT','DOGE/USDT'];
//            return  $this->responseJson(200,"success",[]);
    ...
    public function sysNotice(){
        $noticle = Article::query()->where("category_id",ArticleCategory::$typeMap['notice'])->limit(5)->orderByDesc('updated_at')->get();//1：未读 0：已读
    ...
    public function marketdynamic(Request $request){
        ...
        $move = ArticleCategory::query()->where("id",/*$data["kind"]*/32)->value("id");
        $trends = Article/*Category*/::query()->where(/*"pid"*/'category_id',$move)->orderByDesc("created_at")->limit($request->limit)->get();
    ...
    public function clause(){
        $clause = Article::query()->select("id","category_id")->whereIn("category_id",[10,11])->get();
        if( blank($clause)) return $this->successWithData([],"fail");
        $arr =array();
        $clauses = $clause->makeHidden("translations")->toArray();
        $k =0;
        foreach ( $clauses as $val ){
            unset($val["body"]);
