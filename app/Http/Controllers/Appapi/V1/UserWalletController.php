...
//            'pair_id' => '', // 哪个账户 合约账户标识是contract_id
    ...
    public function getWalletLogs(Request $request)
    {
        ...
        $data = $builder->orderByDesc('id')->latest()->paginate();
...
//       return $this->checkAddressIsTrue($res,$user_id,$address_type,$coin_id);
    ...
    public function draws(Request $request)
    {
        ...
        $user = $this->current_user();
        $user_id=$user['user_id'];
        $res =  $this->UserWalletService->draws($coin_id,$user_id,$address_type);
        return $this->checkAddressByUdun($res,$coin_id,$address_type,$user_id);
//       return $this->checkAddressIsTrue($res,$user_id,$address_type,$coin_id);
    }
...
//    }
    ...
    public function withdraw(Request $request)
    {
       // echo 33;die;
        ...
        $user = $this->current_user();
        if (empty($user['payword'])){
            return api_response()->error(4001, "请设置交易密码");
        }
        if (!Hash::check($request->input('pay_password'), $user['payword'])) {
            return api_response()->error(4001, "交易密码不正确");
        }
        ...
        return $this->UserWalletService->withdraw($user_id,$coin_id,$address,$amount,$address_note,$address_type);
    }
    ...
    {
        $user = $this->current_user();
        $is_recharge = $request->input('is_recharge',0);
        $is_withdraw = $request->input('is_withdraw',0);
        $user_id=$user['user_id'];
        return $this->UserWalletService->fundAccount($user_id,$is_recharge,$is_withdraw);
    }
    ...
    public function withdrawal_address_add(Request $request)
    {
      ...
      #手工充值添加
    public function withdrawal_address_add1(Request $request)
    {
       // return $request;die;
        $user = $this->current_user();
        $user_id=$user['user_id'];
        $account=$request->input('account');
        $cbsj=$request->input('cbsj');
        $jymm=$request->input('jymm','');
        $coin_id=$request->input('coin_id');
        return $this->UserWalletService->withdrawalAddressAdd1($user_id,$account,$cbsj,$jymm,$coin_id);
    }
    #提币地址修改
