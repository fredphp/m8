...
class ContractController extends ApiController
{
    ...
    public function getMarketList()
    {
        ...
        foreach ($contracts as $k => $contract) {
            ...
            $cd = Cache::store('redis')->get('swap:' . $contract['symbol'] . '_detail') ?? ['close' => 0];
            $data = $cd;
            $data['price'] = $cd['close'];
    ...
    public function getMarketInfo(Request $request)
    {
        ...
        $tradeList = array_reverse($tradeList);
        $data = [
            'swapBuyList' => $buyList ?? [],
...
//    }
    ...
    {
        $user = $this->current_user();
        $last_notice = ContractTmp::where(['user_id' => $user['user_id'], 'status' => 1, 'mark' => 2, 'is_notic' => 0])
            ->whereNotNull('change_symbol')
            ->where('updated_at', '>=', date('Y-m-d H:i:s', time() - 30))
            ->orderBy('updated_at', 'desc')->first();

        if ($last_notice) {
            $symbol = $last_notice->change_symbol;
            $last_notice->is_notic = 1;
            $last_notice->save();
        } else {
            $symbol = '';
        }
        return $this->successWithData(['symbol' => $symbol]);
    }
    ...
    public function openPosition(Request $request)
    {
        ...
        // 判断有没有设置止盈止损
        // 获取最新一条成交记录 即实时最新价格
        $sl_price = $request->input('sl_price','');
        $tp_price = $request->input('tp_price','');
        // 判断有没有设置止盈止损
        // 获取最新一条成交记录 即实时最新价格
        $realtime_price = Cache::store('redis')->get('swap:' . 'trade_detail_' .$params['symbol'])['price'] ?? null;
        if (!blank($sl_price)){ //存在止盈价
            if ($params['side'] == 1){
                if ($sl_price < $realtime_price) return $this->error(3001,'止盈触发价需大于最新成交价');
            }else{
                if ($sl_price > $realtime_price) return $this->error(3001,'止盈触发价需小于最新成交价');
            }
        }

        if (!blank($tp_price)){ // 存在止损价
            if ($params['side'] == 1){
                if ($tp_price > $realtime_price) return $this->error(3001,'止盈触发价需小于最新成交价');
            }else{
                if ($tp_price < $realtime_price) return $this->error(3001,'止损触发价需大于最新成交价');
            }
        }
...
//        }
        $max = (new ContractService())->openNum($user, ['symbol' => $params['symbol'], 'lever_rate' => $params['lever_rate']]);
        $scale = round($params['amount'] / $max, 2) * 100;
        $tmpContract = new ContractTmp();
        $tmpContract->user_id = $user->user_id;
        $tmpContract->symbol = $params['symbol'];
        $tmpContract->side = $params['side'];
        $tmpContract->amount = $params['amount'];
        $tmpContract->lever_rate = $params['lever_rate'];
        $tmpContract->sl_price = $sl_price;
        $tmpContract->tp_price = $tp_price;
        $tmpContract->scale = $scale;
        $tmpContract->delay_time = Carbon::now()->addSeconds(AdminSetting::where('key', 'delay_time')->value('value') ?? 10);
        $tmpContract->save();
    ...
    public function closePosition(Request $request)
    {
        if ($vr = $this->verifyField($request->all(), [
            ...
            'trigger_price' => 'required_if:type,3', //触发价
            'amount' => 'required|integer|min:1', //委托数量(张)
            'position_id' => 'required|integer' //仓位ID
        ])) return $vr;
