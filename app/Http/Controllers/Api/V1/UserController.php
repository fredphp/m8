...
//        if ( !$this->isIdentityCard($params['id_card']) ) return $this->error(0,'身份证不合法');
    ...
    public function topAuth(Request $request,UserService $userService)
    {
        ...
        $domain = env('IMG_URL');
        $params['front_img'] = strpos($params['front_img'], $domain) === true ? $params['front_img'] : $domain.'/storage/'.$params['front_img'];
        $params['back_img'] = strpos($params['back_img'], $domain) === true ? $params['back_img'] : $domain.'/storage/'.$params['back_img'];
