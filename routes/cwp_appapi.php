...
$api->group(['namespace' => 'V1','middleware'=>'auth.api'], function ($api) {
    ...
    $api->post('user/walletImage','UserWalletController@wallet_image');//钱包充币二维码
    ...
    $api->post('question','UserController@question');
    $api->post('addRecharge','UserController@addRecharge');
    $api->get('repleList','UserController@repleList');
    $api->get('getScale','UserController@getScale');
    $api->get('successPay','UserController@successPay');
    $api->get('rechargeList','UserController@rechargeList');
    ...
    $api->post('user/withdrawalAddressAdd','UserWalletController@withdrawal_address_add');//提币地址添加
     $api->post('user/withdrawalAddressAdd1','UserWalletController@withdrawal_address_add1');//提币地址添加
