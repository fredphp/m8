...
//    $api->post('user/verifyLogin','LoginController@verifyLogin');//验证码登录
    ...
    //交割合约
    $api->get('time/getKline','TimeContractController@getKline');
...
$api->group(['namespace' => 'V1','middleware'=>'auth.api'], function ($api) {
    ...
    // 签到
    $api->post('user/sign','UserSignController@start_signing'); // 签到 小白
    $api->get('user/sign_list','UserSignController@sign_list'); // 签到列表 小白
    ...
    $api->get('contract/getChange','ContractController@getChange');

    //交割合约
    $api->post('time/pushFollowPlan','TimeContractController@pushFollowPlan');//发起跟单计划
    $api->post('time/followOrder','TimeContractController@followOrder');//跟单
    $api->post('time/placeOrder','TimeContractController@placeOrder');//散户下单
    $api->get('time/planRecord','TimeContractController@planRecord');//跟单计划列表
    $api->get('time/myOrderList','TimeContractController@myOrderList');//我的订单
    $api->get('time/myPlanRecord','TimeContractController@myPlanRecord');//我的跟单计划
    $api->get('time/myFollowRecord','TimeContractController@myFollowRecord');//我的跟单历史
    $api->post('time/cancelFollow','TimeContractController@cancelFollow');//取消跟单
    $api->post('time/delFollowPlan','TimeContractController@delFollowPlan');//删除跟单计划
    $api->get('time/cycleList','TimeContractController@cycleList');//下单周期
    // 永续合约
...
// 质押挖矿
$api->group(['namespace' => 'V1'], function ($api) {
    $api->get('pledge/productList','PledgeProductController@getProductList');
    $api->get('pledge/product','PledgeProductController@getProduct');
    $api->get('pledge/quotation','PledgeProductController@getquotation');
});

$api->group(['namespace' => 'V1','middleware'=>'auth.api'], function ($api) {
    $api->post('pledge/buyProduct','PledgeProductController@buyProduct');
    $api->get('pledge/orderList','PledgeProductController@getOrderList');
    $api->get('pledge/order','PledgeProductController@getOrder');
});
